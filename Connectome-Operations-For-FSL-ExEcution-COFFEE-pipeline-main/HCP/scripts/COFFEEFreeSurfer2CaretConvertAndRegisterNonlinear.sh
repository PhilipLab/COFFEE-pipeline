...
#START230614
#echo -e "\n    **** for LowResMesh in ${LowResMeshes} ; do ****\n"
#echo "    **** for LowResMesh in ${LowResMeshes} ; do ****"
...
#echo "    **** Loop through left and right hemispheres ****"
...
for Hemisphere in L R ; do
	...
	for Surface in white pial ; do
		...
		if [ ! $Secondary = $Type ] ; then
			...
			Secondary=""
                ...
                #START230614
                #echo "here0 mris_convert "$FreeSurferFolder"/surf/"$hemisphere"h."$Surface" "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii"
		...
		mris_convert "$FreeSurferFolder"/surf/"$hemisphere"h."$Surface" "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii
                ...
                #START230614
                #echo "here1 ${CARET7DIR}/wb_command -set-structure "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii ${Structure} -surface-type $Type$Secondary"
	...
	for Map in sulc@sulc@Sulc thickness@thickness@Thickness curv@curvature@Curvature ; do
		...
		mris_convert -c "$FreeSurferFolder"/surf/"$hemisphere"h."$fsname" "$FreeSurferFolder"/surf/"$hemisphere"h.white "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$wbname".native.shape.gii
                ...
                #START230615
                #mris_convert in Freesurfer 7.4.0 prepends files
                f0="$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$wbname".native.shape.gii
                if [ ! -f ${f0} ];then
                    a0=($(ls ${f0%/*}/*${f0##*/}))
                    if((${#a0[@]}>1));then
                        echo "ERROR: More than one match for ${f0%/*}/*${f0##*/}"
                        printf '       %s\n' "${a0[@]}"
                        exit 111
                    fi
                    mv ${a0[0]} ${f0}
